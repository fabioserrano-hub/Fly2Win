...
export default function VLPombal({ carreira, onVoltar, onGuardar, idioma = 'pt' }) {
  ...
  const getEstruturaCarreira = (id) => carreira.estruturas?.[id] || { nivel: 0 }
  ...
  const construir = (est) => {
    ...
    if (carreira.orcamento < custo) {
      setMsg({ tipo:'erro', texto: idioma==='en'?'Not enough budget!':idioma==='es'?'¡Presupuesto insuficiente!':'Orçamento insuficiente!' })
      setTimeout(() => setMsg(null), 2500)
      return
    }

    const novasEstruturas = { ...carreira.estruturas, [est.id]: { nivel: proximoNivel } }
    const novaCarreira = { ...carreira, estruturas: novasEstruturas, orcamento: carreira.orcamento - custo }
    ...
    <div style={{ minHeight:'100vh', background:'#030812', color:'#fff', fontFamily:'inherit' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:'#7A8699' }}>
              {idioma==='en'?'Average level':idioma==='es'?'Nivel medio':'Nível médio'}: {nivelMedio} · {carreira.orcamento.toLocaleString()}€
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:'12px 16px', display:'flex', flexDirection:'column', gap:10 }}>
        ...
        {estruturas.map(est => {
          ...
          const podeComprar = !maxed && carreira.orcamento >= proxCusto
