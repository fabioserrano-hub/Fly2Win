...
function CardMercado({ item, onComprar, onLicitar, orcamento, idioma }) {
  ...
  return (
    <div style={{ background:'linear-gradient(145deg,#0d1428,#070c18)', border:`1px solid ${cor}20`, borderRadius:14, padding:'14px', position:'relative', overflow:'hidden' }}>
      ...
      {item.emLeilao && (
        <div style={{ position:'absolute', top:8, right:8, fontSize:9, background:'rgba(212,175,55,.2)', color:T.gold, padding:'2px 6px', borderRadius:4, fontWeight:700 }}>
      ...
      <div style={{ fontSize:14, fontWeight:800, color:T.text, marginBottom:1 }}>{item.nome}</div>
      <div style={{ fontSize:10, color:T.muted, marginBottom:6 }}>{item.sexo==='M'?'♂':'♀'} · {item.especialidade}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:4, marginBottom:10 }}>
        {['velocidade','resistencia','orientacao'].map(a => (
          <div key={a} style={{ textAlign:'center', padding:'4px', background:T.surface, borderRadius:6 }}>
            <div style={{ fontSize:12, fontWeight:700, color:corAtributo(item.atributos[a]) }}>{item.atributos[a]}</div>
            <div style={{ fontSize:8, color:T.muted }}>{attrL[a]}</div>
          </div>
        ))}
      </div>
      ...
      {item.emLeilao ? (
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
            <div style={{ fontSize:11, color:T.muted }}>{idioma==='en'?'Current bid':idioma==='es'?'Puja actual':'Licitação actual'}</div>
            <div style={{ fontSize:13, fontWeight:700, color:T.gold }}>{item.licitacaoAtual > 0 ? item.licitacaoAtual.toLocaleString()+'€' : (idioma==='en'?'No bids':idioma==='es'?'Sin pujas':'Sem licitações')}</div>
          </div>
          <div style={{ fontSize:10, color:T.muted, marginBottom:8 }}>⏱ {item.tempoRestante}d {idioma==='en'?'remaining':idioma==='es'?'restantes':'restantes'}</div>
          <button onClick={() => onLicitar(item)} disabled={!podeLicitar}
            style={{ width:'100%', padding:'8px', borderRadius:8, border:'none', background: podeLicitar ? 'linear-gradient(135deg,#D4AF37,#B8960C)' : 'rgba(255,255,255,.06)', color: podeLicitar ? T.surface : '#475569', fontSize:11, fontWeight:700, cursor: podeLicitar?'pointer':'default', fontFamily:"'Inter',system-ui,sans-serif" }}>
        ...
        <div>
          ...
          <button onClick={() => onComprar(item)} disabled={!podeComprar}
            style={{ width:'100%', padding:'8px', borderRadius:8, border:'none', background: podeComprar ? 'linear-gradient(135deg,#2DD4A7,#059669)' : 'rgba(255,255,255,.06)', color: podeComprar ? T.surface : '#475569', fontSize:11, fontWeight:700, cursor: podeComprar?'pointer':'default', fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(180deg,#050D1A,#030812)', borderBottom:'1px solid rgba(255,255,255,.05)', padding:'14px 16px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:T.muted }}>{carreira.orcamento.toLocaleString()}€ · {carreira.pombos.length} {idioma==='en'?'pigeons':idioma==='es'?'palomas':'pombos'}</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:6 }}>
          {tabs.map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)}
              style={{ flex:'none', padding:'8px 16px', borderRadius:8, border:tab===id?'none':'1px solid rgba(255,255,255,.08)', background:tab===id?'linear-gradient(135deg,#f97316,#ea580c)':'rgba(255,255,255,.04)', color:tab===id?'#fff':'#cbd5e1', fontSize:12, fontWeight:tab===id?700:500, cursor:'pointer', fontFamily:"'Inter',system-ui,sans-serif", minHeight:36 }}>
              {label}
            </button>
          ))}
        </div>
            ...
            <div style={{ display:'flex', gap:6, marginBottom:12, overflowX:'auto', scrollbarWidth:'none', paddingBottom:4 }}>
              {[['todos',idioma==='en'?'All':idioma==='es'?'Todos':'Todos'],['leilao',idioma==='en'?'Auction':idioma==='es'?'Subasta':'Leilão'],['venda',idioma==='en'?'Buy now':idioma==='es'?'Compra ya':'Compra já'],['elite','⭐ Elite']].map(([id,label]) => (
                <button key={id} onClick={() => setFiltro(id)}
                  style={{ flex:'none', padding:'6px 12px', borderRadius:8, border:filtro===id?'none':'1px solid rgba(255,255,255,.08)', background:filtro===id?'linear-gradient(135deg,#1E5FD9,#1456C0)':'rgba(255,255,255,.04)', color:filtro===id?'#fff':'#cbd5e1', fontSize:11, cursor:'pointer', fontFamily:"'Inter',system-ui,sans-serif", whiteSpace:'nowrap' }}>
                  {label}
                </button>
              ))}
              <button onClick={() => setMercado(gerarMercado(idioma))}
                style={{ flex:'none', padding:'6px 12px', borderRadius:8, border:`1px solid ${T.surface2}`, background:'rgba(255,255,255,.04)', color:T.muted, fontSize:11, cursor:'pointer', fontFamily:"'Inter',system-ui,sans-serif" }}>
            ...
            {carreira.pombos.map(p => {
              ...
              return (
                <div key={p.id} onClick={() => setVenderSel(sel?null:p)}
                  style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', background:sel?`${cor}10`:T.surface, border:`1.5px solid ${sel?cor:'rgba(255,255,255,.06)'}`, borderRadius:10, cursor:'pointer', transition:'all .15s' }}>
                  ...
                  <div style={{ fontSize:12, fontWeight:700, color:T.gold }}>{(p.valor||1000).toLocaleString()}€</div>
                </div>
              )
            })}
            ...
            {venderSel && (
              <div style={{ padding:'14px', background:'rgba(249,115,22,.06)', border:'1px solid rgba(249,115,22,.2)', borderRadius:12, marginTop:4 }}>
                ...
                <div style={{ fontSize:11, color:T.muted, marginBottom:8 }}>
                  {idioma==='en'?'Market value':idioma==='es'?'Valor de mercado':'Valor de mercado'}: <span style={{ color:T.gold, fontWeight:700 }}>{(venderSel.valor||1000).toLocaleString()}€</span>
                </div>
                <input type="number" value={precoVenda} onChange={e => setPrecoVenda(e.target.value)}
                  ...
                  style={{ width:'100%', padding:'10px', background:T.surface, border:'1px solid rgba(255,255,255,.1)', borderRadius:8, color:T.text, fontSize:14, fontFamily:"'Inter',system-ui,sans-serif", outline:'none', boxSizing:'border-box', marginBottom:10 }}/>
                <button onClick={vender} disabled={!precoVenda || isNaN(precoVenda)}
                  style={{ width:'100%', padding:'10px', borderRadius:8, border:'none', background:'linear-gradient(135deg,#f97316,#ea580c)', color:T.text, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:"'Inter',system-ui,sans-serif" }}>
