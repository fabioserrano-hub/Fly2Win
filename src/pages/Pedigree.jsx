...
// Constrói árvore com deteção de consanguinidade
...
export default function Pedigree({ nav, params }) {
  ...
  // Realtime para notificações de atualização
  useEffect(() => {
  ...
  // ── selecionarPombo com memoização ──────────────────────────────────────
  const selecionarPombo = useCallback(async (id) => {
    ...
    // Carregar dados manuais dos ancestrais
    ...
    const geraçõesCompletas = geracoes
    setEstatisticas({
      unicos: unicos.size,
      total: total,
      consanguinidade,
  ...
  // ── trocar pai/mãe (drag & drop simplificado) ──────────────────────────
  ...
  // ── componentes de nó melhorados ──────────────────────────────────────
  const NodeCard = ({ nodeKey, label, destaque, mini, nivel }) => {
