...
function calcConsanguinidade(pomboA, pomboB, todos) {
  // Recolher antepassados até 3 gerações
...
function calcScore(macho, femea, todos, espAlvo) {
  // Percentil médio do casal
  const percentilMedio = ((macho.percentil||0) + (femea.percentil||0)) / 2

  // Complementaridade de especialidade
...
export default function SeleccionadorCasais({ nav }) {
    ...
    setTimeout(() => {
      ...
      machosAlvo.forEach(m => {
        femeas.forEach(f => {
          ...
          pares.push({ macho:m, femea:f, score, consang, percentilMedio, espComum })
        })
      })
  ...
  }, [machos, femeas, pombos, espAlvo, topN, pomboFixo])
