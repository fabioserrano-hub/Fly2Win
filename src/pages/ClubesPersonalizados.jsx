  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:3,background:'#0A1628',borderRadius:10,padding:3,marginBottom:14}}>
        {[['ranking','🏆 Ranking'],['membros','👥 Membros']].map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k)} style={{flex:1,padding:'8px',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer',border:'none',fontFamily:'inherit',background:tab===k?'linear-gradient(135deg,#4C8DFF,#2563EB)':'none',color:tab===k?'#fff':'#475569'}}>{l}</button>
        ))}
      </div>
