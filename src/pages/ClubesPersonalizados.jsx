    ...
    <div>
      ...
      <div style={{display:'flex',gap:3,background:'#0A1628',borderRadius:10,padding:3,marginBottom:14}}>
        {[['meus','🎽 Os meus'],['publicos','🌐 Públicos']].map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k)} style={{flex:1,padding:'8px',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer',border:'none',fontFamily:'inherit',background:tab===v?'linear-gradient(135deg,#D4AF37,#B8960C)':'none',color:tab===v?'#050D1A':'#475569'}}>{l}</button>
        ))}
      </div>
